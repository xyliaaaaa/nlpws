...
# FLAGS.DEFINE_integer("dict_size",5144)
# FLAGS.DEFINE_float("learning_rate",0,1)
# FLAGS.DEFINE_integer("num_layers",2)
# FLAGS.DEFINE_integer("timestep_size",32)
# FLAGS.DEFINE_integer("hidden_size",128)
# FLAGS.DEFINE_integer("input_size",300)
#
FLAGS = None
...
def main():
    ...
    '''
    向节点附加 tf.summary.scalar 操作来分别输出学习速度和期望误差，可以给每个 scalary_summary 分配一个有意义的标签为 'learning rate' 和 'loss function'
    '''
    ...
    if FLAGS.train:
        ...
        if tf.gfile.Exists(FLAGS.summaries_dir):
            # print(tf.gfile.ListDirectory(FLAGS.summaries_dir))
            # tf.gfile.DeleteRecursively('summaries\\')
        ...
        for step in range(int(test_steps)):
            ...
            for i in range(len(x_results)):
                ...
                print(x_result, y_predict_result)
